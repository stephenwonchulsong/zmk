...
#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)
#define MOUSE_PS2_CRASH_RECOVERY_ERROR_THRESHOLD CONFIG_ZMK_INPUT_MOUSE_PS2_CRASH_RECOVERY_ERROR_THRESHOLD
#define MOUSE_PS2_CRASH_RECOVERY_COOLDOWN_MS CONFIG_ZMK_INPUT_MOUSE_PS2_CRASH_RECOVERY_COOLDOWN_MS
#endif
...
struct zmk_mouse_ps2_data {
    ...
    struct k_work crash_recovery_work;
#endif
};
...
#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)
void zmk_mouse_ps2_crash_recovery_inc_error(const struct device *dev);
void zmk_mouse_ps2_crash_recovery_reset_error(const struct device *dev);
void zmk_mouse_ps2_crash_recovery_work_handler(struct k_work *work);
#endif
...
#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)
    zmk_mouse_ps2_crash_recovery_inc_error(dev);
#endif
}
...
#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)
    // Reset error counter on successful packet processing
    zmk_mouse_ps2_crash_recovery_reset_error(data->dev);
#endif
}
...
#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)
    k_work_init(&data->crash_recovery_work, zmk_mouse_ps2_crash_recovery_work_handler);
    data->consecutive_errors = 0;
    data->consecutive_successes = 0;
    data->last_recovery_time = 0;
#endif
 ...
 * Crash Recovery
 *
 * When enabled, monitors for consecutive errors during active use and
 * automatically toggles the reset pin to recover from trackpoint crashes.
 */

#if IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY)

// Require 5 consecutive successful packets before resetting error counter
#define CRASH_RECOVERY_SUCCESS_THRESHOLD 5
...
void zmk_mouse_ps2_crash_recovery_reset_error(const struct device *dev) {
    ...
    // Only reset error counter after multiple consecutive successes
    if (data->consecutive_successes >= CRASH_RECOVERY_SUCCESS_THRESHOLD) {
        if (data->consecutive_errors > 0) {
            LOG_DBG("Crash recovery: %d consecutive successes, resetting error count from %d",
                    data->consecutive_successes, data->consecutive_errors);
        }
        data->consecutive_errors = 0;
        data->consecutive_successes = 0;
    }
...
void zmk_mouse_ps2_crash_recovery_inc_error(const struct device *dev) {
    ...
    data->consecutive_successes = 0;  // Reset success streak on any error

    LOG_DBG("Crash recovery: consecutive errors = %d", data->consecutive_errors);

    if (data->consecutive_errors >= MOUSE_PS2_CRASH_RECOVERY_ERROR_THRESHOLD) {
        // Check if reset pin is configured
        if (!config->has_rst_gpio) {
            LOG_WRN("Crash recovery threshold reached but no reset pin configured");
            data->consecutive_errors = 0;
            return;
        }
        ...
        // Check cooldown
        int64_t now = k_uptime_get();
        if (data->last_recovery_time > 0 &&
            (now - data->last_recovery_time) < MOUSE_PS2_CRASH_RECOVERY_COOLDOWN_MS) {
            LOG_WRN("Crash recovery: still in cooldown period");
            data->consecutive_errors = 0;
            return;
        }

        LOG_WRN("Crash recovery: error threshold reached (%d errors), scheduling recovery",
                data->consecutive_errors);

        data->consecutive_errors = 0;
        data->last_recovery_time = now;

        // Submit work to perform recovery (can't do blocking operations in callback context)
        k_work_submit(&data->crash_recovery_work);
    }
}

void zmk_mouse_ps2_crash_recovery_work_handler(struct k_work *work) {
    struct zmk_mouse_ps2_data *data = CONTAINER_OF(work,
                                                    struct zmk_mouse_ps2_data,
                                                    crash_recovery_work);
    const struct device *dev = data->dev;
    const struct zmk_mouse_ps2_config *config = dev->config;
    ...
    LOG_WRN("Crash recovery: starting reset sequence...");

    // Disable reporting during recovery
    zmk_mouse_ps2_activity_reporting_disable(dev);

    // Toggle reset pin: set HIGH, wait, set LOW
    LOG_INF("Crash recovery: toggling reset pin...");

    err = gpio_pin_set_dt(&data->rst_gpio, 1);
    if (err) {
        LOG_ERR("Crash recovery: failed to set reset pin HIGH (err %d)", err);
    ...
    if (err) {
        LOG_ERR("Crash recovery: failed to set reset pin LOW (err %d)", err);
        goto recovery_failed;
    }

    // Wait for mouse to re-initialize
    LOG_INF("Crash recovery: waiting for mouse to reinitialize...");
    err = zmk_mouse_ps2_init_wait_for_mouse(dev);
    if (err) {
        LOG_ERR("Crash recovery: mouse failed to reinitialize after reset");
        goto recovery_failed;
    }

    // Reconfigure sampling rate if not default
    if (config->sampling_rate != MOUSE_PS2_CMD_SET_SAMPLING_RATE_DEFAULT) {
        LOG_INF("Crash recovery: restoring sampling rate to %d...", config->sampling_rate);
        zmk_mouse_ps2_set_sampling_rate(dev, config->sampling_rate);
    }

    // Re-enable scroll mode if it was enabled
    if (data->packet_mode == MOUSE_PS2_PACKET_MODE_SCROLL) {
        LOG_INF("Crash recovery: re-enabling scroll mode...");
        zmk_mouse_ps2_set_packet_mode(dev, MOUSE_PS2_PACKET_MODE_SCROLL);
    }

    // Restore trackpoint sensitivity settings
    if (data->is_trackpoint) {
        LOG_INF("Crash recovery: restoring trackpoint settings...");

        if (data->tp_sensitivity != MOUSE_PS2_CMD_TP_SET_SENSITIVITY_DEFAULT) {
            LOG_INF("Crash recovery: restoring sensitivity to %d", data->tp_sensitivity);
            zmk_mouse_ps2_tp_sensitivity_set(dev, data->tp_sensitivity);
        }

        if (data->tp_neg_inertia != MOUSE_PS2_CMD_TP_SET_NEG_INERTIA_DEFAULT) {
            LOG_INF("Crash recovery: restoring neg inertia to %d", data->tp_neg_inertia);
            zmk_mouse_ps2_tp_neg_inertia_set(dev, data->tp_neg_inertia);
        }

        if (data->tp_value6 != MOUSE_PS2_CMD_TP_SET_VALUE6_UPPER_PLATEAU_SPEED_DEFAULT) {
            LOG_INF("Crash recovery: restoring value6 to %d", data->tp_value6);
            zmk_mouse_ps2_tp_value6_upper_plateau_speed_set(dev, data->tp_value6);
        }

        if (data->tp_pts_threshold != MOUSE_PS2_CMD_TP_SET_PTS_THRESHOLD_DEFAULT) {
            LOG_INF("Crash recovery: restoring PTS threshold to %d", data->tp_pts_threshold);
    ...
    // Re-enable reporting
    LOG_INF("Crash recovery: re-enabling data reporting...");
    err = zmk_mouse_ps2_activity_reporting_enable(dev);
    if (err) {
        LOG_ERR("Crash recovery: failed to re-enable reporting (err %d)", err);
        goto recovery_failed;
    }

    LOG_WRN("Crash recovery: completed successfully!");
    return;

recovery_failed:
    LOG_ERR("Crash recovery: FAILED - trackpoint may be non-functional");
    // Try to at least re-enable reporting
    zmk_mouse_ps2_activity_reporting_enable(dev);
}

#endif /* IS_ENABLED(CONFIG_ZMK_INPUT_MOUSE_PS2_ENABLE_CRASH_RECOVERY) */
