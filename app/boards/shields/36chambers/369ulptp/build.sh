...
# settings_reset is board-level, not per-split-half: all three halves would
# build the identical firmware (same board, no per-half overlay). Build it once.
echo "=== Building settings_reset firmware ==="
run "build: settings_reset"      west build -d build/settings_reset    -p -b seeeduino_xiao_ble -- -DSHIELD=settings_reset
...
run "copy: settings_reset.uf2"   cp build/settings_reset/zephyr/zmk.uf2   "$OUT_DIR/settings_reset.uf2"
